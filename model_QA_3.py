...
def model_performance_QA():
    ...
    for sample_name in os.listdir(root_dir):
        ...
        save_prediction(output_path, affine, header, pred_up, image_path)
...
def save_prediction(output_path, affine, header, pred_up, image_path):
    pred_np = pred_up.squeeze().cpu().numpy().astype(np.uint8)  # [D, H, W]
    
    # 创建 TorchIO 图像
    pred_image = tio.ScalarImage(tensor=pred_np[np.newaxis], affine=affine)  # 添加批次维度
    
    # 重采样到参考图像
    ref_img = tio.ScalarImage(image_path)
    resampler = tio.Resample(ref_img)
    pred_resampled = resampler(pred_image)
    
    # 保存
    pred_resampled.save(output_path)
