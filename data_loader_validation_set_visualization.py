...
class HipDataset_Extra(Dataset):
    def __init__(self, root_dir, transform=None):
        self.root_dir=root_dir
        self.sample_dirs = [d for d in os.listdir(root_dir) if os.path.isdir(os.path.join(root_dir, d))]
        self.transform = transform       #transform augmentation function for training dataset
    ...
    def __getitem__(self, idx):
        ...
        # #debugging code
        # print(f"[DEBUG] Mask dtype: {mask.dtype}, Max label: {mask.max()}, Min label: {mask.min()}")
...
def dataloader(Args):
    ...
    root_dir_new_validation = "/banana/yuyang/extra_validation_set"
    dataset2 = HipDataset_Extra(root_dir_new_validation)
