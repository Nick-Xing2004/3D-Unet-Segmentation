...
class HipDataset_Transfer_Learning(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(sample_dir, f"{self.sample_dirs[idx]}_image.nii.gz")
        image = nib.load(image_path).get_fdata() 
        #sample mask loading
        mask_path = os.path.join(sample_dir, f"{self.sample_dirs[idx]}_label.nii.gz")
...
def dataloader_transfer_learning(Args):
    """
    ...
    root_dir = "/banana/yuyang/CT_Pelvic1K_6/CLINIC"
    dataset = HipDataset_Transfer_Learning(root_dir)
