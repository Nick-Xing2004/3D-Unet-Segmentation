...
class UNet3D_3(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=6, init_features=32):
        ...
        #sixth layer  #in_channels=512 -> out_channels=1024
        self.encoder6 = self._block(features * 16, features * 32)
        self.pool6 = nn.MaxPool3d(kernel_size=2, stride=2)

        #bottleneck part
        self.bottleneck = self._block(features * 32, features * 64)   #number of out-channels 2048 (largest receptive field)

        #decoder part
        self.upconv6 = nn.ConvTranspose3d(features * 64, features * 32, kernel_size=2, stride=2)
        self.decoder6 = self._block(features * 64, features * 32)
    ...
    def forward(self, x):
        ...
        bottleneck = self.bottleneck(self.pool6(enc6))      #features * 64

        #decoding and up-sampling
        dec6 = self.upconv6(bottleneck)   #features * 64 -> features * 32
        dec6 = torch.cat([dec6, enc6], dim=1)
        dec6 = self.decoder6(dec6)        #features * 32

        dec5 = self.upconv5(dec6)   #features * 32 -> features * 16
